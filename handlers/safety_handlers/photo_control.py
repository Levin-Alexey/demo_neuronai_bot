...
async def photo_control_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<b>Инструкция:</b>\n"
        "Пожалуйста, отправьте фотографию одного из следующего:\n\n"
        "✅ <b>Принимаются:</b>\n"
        "• Селфи в средствах защиты (каска, жилет и т.д.)\n"
        "• Фото рабочей зоны с видимыми мерами безопасности\n"
        "• Документы о разрешении на работу\n\n"
        "❌ <b>Не принимаются:</b>\n"
        "• Текстовые сообщения\n"
        "• Размытые или неразборчивые фото\n\n"
        "🔍 Я проанализирую изображение и выдам <b>допуск</b> или укажу на нарушения.",
        parse_mode="HTML",
...
async def photo_control_invalid_input(message: types.Message):
    ...
    if message.text and message.text != "❌ Отмена":
        await message.answer(
            "📸 <b>Пожалуйста, отправьте фотографию</b>, а не текст.\n\n"
            "Система ждет визуального подтверждения для анализа безопасности.",
            parse_mode="HTML",
        )
    else:
        await message.answer(
